...
bool operator < (const mov &move_1, const mov &move_2)
{
    if (move_1.src == move_2.src)
        return move_1.dst < move_2.dst;
    else if (move_1.dst == move_2.dst);
        return move_1.src < move_2.src;
}
...
{
    set<mov> moves;
    string enemy_color = get_enemy_color(c);

    set<mov> defr_moves = get_moves(P, c);
    set<mov> atkr_moves = get_moves(P, enemy_color);

    bool mate = checkmate(P, c, defr_moves, atkr_moves);
    if (mate)
        return moves;
    mate = stalemate(P, c, defr_moves, atkr_moves);
    if (mate)
        return moves;

    moves = defr_moves;

    string king_src = get_king(P, c);
    add_castle(king_src, moves, P, c, atkr_moves);

    set<mov> filtered_moves = filter_check(P, c, moves,
                                           atkr_moves);
    set<mov> kingless = filter_king(P, c, filtered_moves);
...
int main()
{
    ...
    set <mov> moves = legalMoves(board, "W");
