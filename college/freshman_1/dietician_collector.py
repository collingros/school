...
def get_clients(filename="default.txt"):
	...
	for row in reader:	#read in file and splice information into cilents dict
		try:
			clients[row[0]] = [row[1].strip(), row[2].strip(), row[3].strip(), row[4].strip(), row[5].strip()] #client name, body type, weight, height, age, level of activity
		except: # to skip the first empty line
			break
		print(row[0].strip())
	while 1:
