...
/*
void cvt_board(position &board)
{
    string puzzle = ""
    position new_board = create_board();
...
{
//    string def_pos = "BK8a Bp7a Bp7b Bp7c WK1h Wp2h Wp2g Wr1f";
//    string def_pos = "WK1h   Bk3f  Wp4h  BK5h  Wb6e  WQ6f  Br8h";
//    string def_pos = "Br8a Wb6e WQ7h Wp4h BK5h Bk3f WK1h";
    string def_pos = "WK1h   Bk3f  Wp4h  BK5h  Wb6e  WQ6f  Br8h";
...
color get_enemy_color_c(color c)
{
    if (c == 'W')
        return 'B';
    else
...
bool check(position board, string color, set<mov> atkr_moves)
{
    ...
    for (auto move : atkr_moves) {
        string dst_sqr = string(move.dst);
...
bool win(position board, int num_moves, color ply_c)
{
    set<mov> white_moves = legalMoves(board, 'W');
    set<mov> black_moves = legalMoves(board, 'B');
    ...
    if (ply_c == 'W') {
        for (auto move : white_moves) {
            position future_board = board;
            fast_forward(future_board, move);

            if (win(future_board, num_moves - 1, 'B')) {
                return true;
            }
        }
    ...
    for (auto move : black_moves) {
        ...
        if (!win(future_board, num_moves, 'W')) {
            return false;
        }
    }
...
{
    set<mov> white_moves = legalMoves(s, 'W');

    for (auto move : white_moves) {

        position future_board = s;
        fast_forward(future_board, move);

        if (win(future_board, n - 1, 'B')) {
            struct mov new_move = cvt_move(move);
            return new_move;
        }
    }
